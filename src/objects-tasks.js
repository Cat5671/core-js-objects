...
function compareObjects(obj1, obj2) {
  ...
  return entriesObj1.every(([key, value]) => obj2[key] === value);
}
...
function makeWord(lettersObject) {
  const arrLetter = Object.entries(lettersObject);
  return arrLetter
    .reduce((acc, [key, arrInd]) => {
      arrInd.forEach((value) => {
        acc[value] = key;
      });
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    el = queue[i];
    if (el === 25) money['25'] += 1;
    else if (el === 50 && money['25'] !== 0) {
      money['25'] -= 1;
      money['50'] += 1;
    } else if (el === 100 && money['50'] !== 0 && money['25'] !== 0) {
      money['50'] -= 1;
      money['25'] -= 1;
    } else if (el === 100 && money['25'] > 2) {
      money['25'] -= 3;
    } else return false;
  }
  return true;
...
function fromJSON(proto, json) {
  ...
  Object.setPrototypeOf(parseJSON, proto);
  return parseJSON;
}
